...
namespace Bmon.Client.Core.Config.v1_0_0_0
{
    ...
    {
        public List<Lib.Models.LocalFileConfig> MyLocalFiles = new List<Lib.Models.LocalFileConfig>();
            ...
            try
            {
                    ...
                    && reader.GetAttribute("Version") == "1.0.0.0")
                {
                    ...
                    while (reader.MoveToContent() == XmlNodeType.Element
                        && reader.LocalName == (typeof(Lib.Models.LocalFileConfig).Name))
                    {
                        var config = new XmlSerializer(typeof(Lib.Models.LocalFileConfig));
                        MyLocalFiles.Add((Lib.Models.LocalFileConfig)config.Deserialize(reader));
                    }
                }
                else
        ...
        public void WriteXml(XmlWriter writer)
        {
            try
            {
                ...
                {
                    var thing = new XmlSerializer(typeof(Lib.Models.LocalFileConfig));
        ...
        public override string ToString()
        {
            ...
            foreach (var config in MyLocalFiles)
            {
                output.Append(typeof(Lib.Models.LocalFileConfig).Name + Environment.NewLine);
